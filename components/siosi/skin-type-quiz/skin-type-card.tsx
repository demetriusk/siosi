...
export function SkinTypeCard({ skinTypeCode, onTakeQuizAction, onRetakeAction, isLoading = false }: SkinTypeCardProps) {
    ...
    return (
      <Card className="border-[#E5E7EB] shadow-none">
        <CardHeader className="space-y-2 pb-4">
          <p className="text-xs uppercase tracking-[0.2em] text-[#9CA3AF]">Skin Type</p>
          <CardTitle className="text-xl font-semibold tracking-tight text-[#0A0A0A]">Loading your skin type…</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col gap-3">
            <div className="h-4 w-2/3 animate-pulse rounded-sm bg-[#E5E7EB]" />
          ...
          <div className="space-y-4">
            <div className="rounded-sm border border-[#E5E7EB] bg-[#F9FAFB] p-4">
              <div className="flex flex-wrap items-center gap-2 text-sm font-medium text-[#0A0A0A]">
                <span className="rounded-sm bg-[#0A0A0A] px-2 py-1 text-xs uppercase tracking-wide text-white">{profile.code}</span>
                <span>{profile.oil_level}</span>
                <span aria-hidden="true">·</span>
                <span>{profile.concern}</span>
                <span aria-hidden="true">·</span>
                <span>{profile.undertone} Undertone</span>
              </div>
              <p className="mt-3 text-sm leading-6 text-[#4B5563]">{profile.description}</p>
            </div>
                    ...
                    {(Object.entries(profile.makeup_needs) as [string, string[]][]).map(([category, items]) => (
                      <div key={category} className="rounded-sm border border-[#E5E7EB] bg-white p-4">
                        ...
                        <ul className="mt-2 space-y-1 text-sm text-[#374151]">
                          {items.length > 0 ? items.map((item) => <li key={item}>{item}</li>) : <li className="text-[#9CA3AF]">No recommendations yet.</li>}
                        </ul>
                      </div>
                    ))}
                  </div>
